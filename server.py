...
class MyHttp(SimpleHTTPRequestHandler):
    ...
    def dispatch(self, http_method):
        req = custom_types.HttpRequest.build(self.path, method=http_method, headers=self.headers)
        ...
        endpoints = {
            ...
            '/films/': [self.handle_static, [f'films.html', req.content_type]],
            '/show/': [self.handle_static, [f'show.html', req.content_type]],
            '/rating/': [self.handle_static, [f'rating.html', req.content_type]],
            '/contact/': [self.handle_static, [f'contact.html', req.content_type]],
        }
    ...
    def send_header_session(self, session: Optional[str]) -> None:
        ...
        if session:
            header = utils.build_session_headers(session)
        else:
            header = utils.build_session_headers('expires', expires=True)
